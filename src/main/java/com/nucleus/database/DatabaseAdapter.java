...
public class DatabaseAdapter {
  ...
  public String create(
    ...
    if (associationUpdates != null && !associationUpdates.isEmpty()) {
      ...
      modifiedCount =
          updateAssociations(query, collectionName, associationUpdates, client).longValue();
      if (modifiedCount == 0) {
  ...
  public boolean setDefault(Bson query, boolean defaultDoc, String collectionName) {
    UpdateResult result =
        getCollection(collectionName).updateOne(query, Updates.set(Fields.DEFAULT, defaultDoc));
  ...
  public Long update(
      Bson query, Map<String, Object> updates, List<Document> arrayFilters, String collectionName) {
  ...
  public Long update(
    ...
    if (!updatesBson.isEmpty()) {
      result =
          getCollection(collectionName).updateMany(query, Updates.combine(updatesBson), option);
      ;
    }
  ...
  private Integer updateAssociations(
      Bson query,
      String collectionName,
      List<AssociationUpdates> associationUpdates,
      String client) {
    FindIterable<Document> ids =
        getCollection(collectionName).find(query).projection(Projections.include(_ID));
    ...
    NucleusLogger.info(
        "Updated " + result.getModifiedCount() + " association docs.", this.getClass());
    return result.getModifiedCount() > 0 ? result.getModifiedCount() : result.getInsertedCount();
  }

  private void updateWriteRequests(
      List<WriteModel<Document>> requests,
      AssociationUpdates associationUpdate,
      String parentEntityDocId,
      String client) {
    ...
    for (Map<String, Object> refEntityIdsMap : associationUpdate.getRefEntityIdsMapList()) {
      ...
      if (refEntityIdsMap.containsKey(Fields.PREVIOUS_VALUE)) {
        ...
        Bson update =
            Updates.set(Fields.ASS_MAPPING, Arrays.asList(newRefEntityDocId, parentEntityDocId));
        Bson query =
            Filters.and(
                Filters.eq(Fields.ASS_NAME, parentEntityName),
                Filters.eq(Fields.ASS_NAME, refEntityName),
                Filters.eq(Fields.ASS_MAPPING, parentEntityDocId),
                Filters.eq(Fields.ASS_MAPPING, prevRefEntityDocId));
  ...
  public Integer deleteAssociations(
      List<String> parentIds, List<AssociationUpdates> associationUpdates) {
    List<WriteModel<Document>> requests = new ArrayList<WriteModel<Document>>();
      ...
      for (Map<String, Object> refEntityIdsMap : associationUpdate.getRefEntityIdsMapList()) {
        ...
        Bson query =
            Filters.and(
                Filters.eq(Fields.ASS_NAME, refEntityName),
                Filters.eq(Fields.ASS_NAME, parentEntityName),
                Filters.eq(Fields.ASS_MAPPING, refEntityDocId),
                Filters.in(Fields.ASS_MAPPING, parentIds));
  ...
  public Long deleteInArray(
      Bson query,
      Integer existingMaxSerial,
    ...
    if (!deletedSerialMap.isEmpty() && existingMaxSerial > 1) {
      Map<String, Object> fieldNewSerialsMap =
          getSerialMapToUpdate(deletedSerialMap, existingMaxSerial, arrayFilters);
  ...
  private void addDeletedSerials(
      String field, Object val, Map<String, List<Integer>> deletedSerialMap) {
  ...
  private Map<String, Object> getSerialMapToUpdate(
      Map<String, List<Integer>> deletedSerialMap,
      Integer existingMaxSerial,
      List<Document> arrayFilters) {
    Document lastFilter =
        arrayFilters.isEmpty()
            ? null
            : arrayFilters.get(arrayFilters.size() - 1); // eg: e1.serial=5
    String filterKey =
        lastFilter == null
            ? "e1.serial"
            : lastFilter.keySet().stream().findFirst().orElse("e1.serial");
  ...
  public Long updateArray(
      Bson query, List<Bson> updatesBson, List<Document> arrayFilters, String collectionName) {
    UpdateOptions option = new UpdateOptions();
    if (arrayFilters != null && !arrayFilters.isEmpty()) {
      option.arrayFilters(arrayFilters);
    }
  ...
  public List<Map<String, Object>> get(
      Bson query, List<String> returnFields, String collectionName) {
    List<Map<String, Object>> docs =
        getCollection(collectionName)
            .find(query)
            .projection(Projections.include(returnFields))
